...
public class OrderController {
    ...
    public ServerResponse create(HttpSession session, @RequestParam("shipping_id") long shippingId) {
    ...
    public ServerResponse<String> cancel(HttpSession session, @RequestParam("order_no") long orderNo) {
        User user = (User) session.getAttribute(Const.CURRENT_USER);
        if (user == null) {
            return ServerResponse.buildErrorByCode(ResponseCode.NEED_LOGIN.getCode(), ResponseCode.NEED_LOGIN.getDesc());
        }
    ...
    public ServerResponse getDetails(HttpSession session, @RequestParam("order_no") Long orderNo) {
        User user = (User) session.getAttribute(Const.CURRENT_USER);
        if (user == null) {
            return ServerResponse.buildErrorByCode(ResponseCode.NEED_LOGIN.getCode(), ResponseCode.NEED_LOGIN.getDesc());
        }
    ...
    public ServerResponse pay(HttpSession session, @RequestParam("order_no") long orderNo, HttpServletRequest request) {
        User user = (User) session.getAttribute(Const.CURRENT_USER);
        if (user == null) {
            return ServerResponse.buildErrorByCode(ResponseCode.NEED_LOGIN.getCode(), ResponseCode.NEED_LOGIN.getDesc());
        }
    ...
    public ServerResponse<Boolean> queryOrderPayStatus(HttpSession session, @RequestParam("order_no") Long orderNo) {
        User user = (User) session.getAttribute(Const.CURRENT_USER);
        if (user == null) {
            return ServerResponse.buildErrorByCode(ResponseCode.NEED_LOGIN.getCode(), ResponseCode.NEED_LOGIN.getDesc());
        }
